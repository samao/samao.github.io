$(() => {
	...
	//------video jq
	let video = $(`<video id="video" preload="none" x-webkit-airplay="allow" webkit-playsinline="true" playsinline="true"></video>`);
	video.css({
		'width':'100%',
		'height':'100%',
		'object-fit':'contain',
	...
	let ready = () => {
		...
		$('body').append(`<span style="color:white;"> ${navigator.userAgent} </br> ${Array.from(video.get(0).attributes).map(e => e.name + '=' + e.value)}</span>`)
	}
